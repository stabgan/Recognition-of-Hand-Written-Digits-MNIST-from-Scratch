...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    a = np.dot(theta,X.transpose())/temp_parameter
    b = np.amax(a,axis = 0)
    c = a-b
    exp = np.exp(c)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    probabilities = np.clip(compute_probabilities(X, theta, temp_parameter), eps, 1 - eps)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    diff = probability - A
    grad = -1/(temp_parameter*n)*(np.dot(X.T, diff.T))
    theta += alpha * (grad.T - lambda_factor * theta)
    return theta
